...
import type { VsysAddress } from '../../types/vsys';
...
export default class VsysGetAddress extends AbstractMethod {
    ...
    constructor(message: CoreMessage) {
        ...
        // set info
        if (bundle.length === 1) {
            this.info = `Export VSYS address for account index #${ fromHardened(this.params[0].path[2]) }`;
        } else {
            this.info = 'Export multiple VSYS addresses';
        }
    }
    ...
    async run(): Promise<VsysAddress | Array<VsysAddress>> {
        const responses: Array<VsysAddress> = [];
        ...
        for (let i = 0; i < this.params.length; i++) {
            ...
            if (batch.showOnTrezor) {
                const silent = await this.device.getCommands().vsysGetAddress(
                    batch.path,
                    false
                );
            ...
            const response = await this.device.getCommands().vsysGetAddress(
                batch.path,
                batch.showOnTrezor
            );
            responses.push({
